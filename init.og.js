...
(function() {
	...
	atheos.Outline = (function() {
		...
		Outline.prototype.initOutlineListContainer = function() {
			...
			this.jQuery('#editor-bottom-bar').append(OutlineMenu);
			this.$OutlineButton = this.jQuery('#OutlineButton');
			this.$OutlineMenu = this.jQuery(OutlineMenu);

			// this.atheos.common.initMenuHandler(this.$OutlineButton, OutlineMenu.el);
			
			this.$OutlineButton.on('click', this.$OutlineMenu.show());
			...
			this.$OutlineMenu.on('click', 'li a', function(element) {
				...
				if (line) {
					_this.atheos.active.gotoLine(line);
					return _this.atheos.active.focus();
				}
			});
		...
		Outline.prototype.updateOutline = function() {
			...
			content = this.atheos.editor.getContent();
			loc = content.split(/\r?\n/);
			matches = [];
			editorOutline = [];
			...
			if (matches.length > 0) {
				...
				editor = this.atheos.editor.getActive().getSession();
	...
	new atheos.Outline(this, jQuery);
